...
export default function SearchPage() {
  ...
  const { data: results = [], isLoading } = useProperties(filters, user?.id);

  // eslint-disable-next-line react-hooks/exhaustive-deps
  const debouncedSearch = useCallback(
    debounce((q: string) => setFilters({ query: q || undefined }), 400),
    [],
  );
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-128px)]">
      {/* Search bar */}
      <div className="px-4 pt-4 pb-3 glass border-b border-warm-200 sticky top-0 z-20">
        <div className="flex items-center gap-2 p-3 rounded-2xl bg-warm-100 mb-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={cn(
              ...
              showFilters
                ...
                : 'bg-white border-warm-200 text-slate-600 hover:border-slate-300',
            )}
          >
          ...
          <div className="flex items-center bg-warm-100 rounded-xl p-1 ml-auto">
            <button
      ...
      {viewMode === 'list' ? (
        <div className="flex-1 overflow-y-auto px-4 pt-4 space-y-4 pb-4">
          {isLoading ? (
            ...
            <>
              <p className="text-xs text-slate-400 font-medium">{results.length} homes found</p>
              {results.length === 0 ? (
